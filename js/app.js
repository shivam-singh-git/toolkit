...
const TOOLS = [
  ...
  { id:'split-pdf', name:'Split / Extract PDF', desc:'Extract specific pages from a PDF. Choose page ranges or individual pages.', icon:'✂️', category:'pdf', tag:'Popular', accentColor:'#f97316' },
...
R['merge-pdf']=(c)=>{
  ...
  c.innerHTML=`<div class="tool-panel"><div class="tool-panel-title">Upload PDF Files</div>
    <div class="drop-zone" id="mDrop"><input type="file" id="mFile" accept=".pdf" multiple/><div class="drop-zone-text"><strong>Click to upload</strong> or drag & drop<br><span style="font-size:12px;color:var(--text-muted)">Select multiple PDF files</span></div></div>
...
R['split-pdf']=(c)=>{
  let lb=null;
  c.innerHTML=`<div class="tool-panel"><div class="tool-panel-title">Upload PDF</div><div class="drop-zone" id="sDrop"><input type="file" id="sFile" accept=".pdf"/><div class="drop-zone-text"><strong>Click to upload</strong> a PDF to split</div></div><div id="sInfo" style="display:none;margin-top:12px"><div class="stats-grid" id="sStats"></div></div></div>
  <div id="sOpt" style="display:none"><div class="tool-panel"><div class="tool-panel-title">Extract Pages</div><label class="field-label">Page Range (e.g. 1-3, 5, 8-10)</label><input type="text" class="field-input" id="sRange"/><div class="btn-group"><button class="btn btn-primary" id="sBtn">✂️ Extract</button><button class="btn btn-secondary" id="sAll">📋 Split All</button></div></div>
...
R['image-to-pdf']=(c)=>{
  ...
  function rl(){$('i2pList').innerHTML=imgs.map((f,i)=>`<li class="file-item"><span class="file-item-icon">🖼️</span><div class="file-item-info"><div class="file-item-name">${f.name}</div><div class="file-item-size">${fmtB(f.size)}</div></div><button class="file-item-remove" onclick="window._i2pRm(${i})">✕</button></li>`).join('');$('i2pOpt').style.display=imgs.length?'block':'none';}
...
// ══ TEXT TO PDF ══
R['text-to-pdf']=(c)=>{
  c.innerHTML=`<div class="tool-panel"><div class="tool-panel-title">Enter Text</div><textarea class="field-input" id="t2pIn" rows="10" style="font-family:var(--font-body)" placeholder="Type or paste text here..."></textarea></div>
...
R['image-compressor']=(c)=>{
  ...
  function comp(){const r=new FileReader();r.onload=e=>{const img=new Image();img.onload=()=>{const cv=document.createElement('canvas');cv.width=img.width;cv.height=img.height;cv.getContext('2d').drawImage(img,0,0);
    cv.toBlob(blob=>{const sv=((1-blob.size/sf.size)*100).toFixed(1);$('icStats').innerHTML=`<div class="stat-card"><div class="stat-val">${fmtB(sf.size)}</div><div class="stat-label">Original</div></div><div class="stat-card"><div class="stat-val">${fmtB(blob.size)}</div><div class="stat-label">Compressed</div></div><div class="stat-card"><div class="stat-val" style="color:var(--accent)">${sv}%</div><div class="stat-label">Saved</div></div>`;
...
R['bg-remover']=(c)=>{
  ...
  <div class="slider-wrap"><span style="font-size:12px;color:var(--text-muted)">Tight</span><input type="range" id="brThr" min="10" max="120" value="50"/><span style="font-size:12px;color:var(--text-muted)">Wide</span><span class="slider-val" id="brThrV">50</span></div>
  <p style="font-size:12px;color:var(--text-muted);margin-top:8px">Adjust sensitivity to capture more or less of the background color. Click the image corners to pick background color.</p>
...
R['image-resizer']=(c)=>{
  ...
  <div id="irOrigInfo" style="margin-bottom:12px;font-family:var(--font-mono);font-size:12px;color:var(--text-muted)"></div>
...
R['case-converter']=(c)=>{
  ...
  <div style="position:relative"><div class="output-area" id="ccOut" style="font-size:14px;font-family:var(--font-body)"></div><button class="copy-btn" onclick="copyText($('ccOut').textContent)">Copy</button></div></div>`;
  window._cc=(mode)=>{
...
R['lorem-ipsum']=(c)=>{
  ...
  <div class="tool-panel" id="liRes" style="display:none"><div class="tool-panel-title">Result</div><div style="position:relative"><div class="output-area" id="liOut" style="font-family:var(--font-body);font-size:14px;line-height:1.8"></div><button class="copy-btn" onclick="copyText($('liOut').textContent)">Copy</button></div></div>`;
...
R['color-palette']=(c)=>{
  ...
  <div class="btn-group"><button class="btn btn-primary" onclick="window._pal()">🎲 Random</button><button class="btn btn-secondary" onclick="window._pal('warm')">🔥 Warm</button><button class="btn btn-secondary" onclick="window._pal('cool')">❄️ Cool</button><button class="btn btn-secondary" onclick="window._pal('pastel')">🌸 Pastel</button><button class="btn btn-secondary" onclick="window._pal('mono')">⬛ Mono</button></div></div>
  ...
  window._pal=(m)=>{const cs=[],bh=Math.random()*360;for(let i=0;i<5;i++){let h,s,l;if(m==='warm'){h=(bh+i*15)%60;s=60+Math.random()*30;l=40+Math.random()*30;}else if(m==='cool'){h=(bh+i*15)%60+180;s=50+Math.random()*30;l=35+Math.random()*35;}else if(m==='pastel'){h=(bh+i*72)%360;s=50+Math.random()*20;l=75+Math.random()*15;}else if(m==='mono'){h=bh;s=20+Math.random()*40;l=15+i*16;}else{h=(bh+i*72+Math.random()*20)%360;s=50+Math.random()*40;l=35+Math.random()*35;}cs.push(h2h(h,s,l));}
    window._palC=cs;$('palD').innerHTML=cs.map(c=>`<div class="palette-swatch" style="background:${c}" onclick="copyText('${c}')"><span>${c}</span></div>`).join('');$('palH').innerHTML=cs.map(c=>`<button class="btn btn-secondary btn-sm" onclick="copyText('${c}')" style="font-family:var(--font-mono);font-size:12px">${c}</button>`).join('');};
  window._pal();
};
